...
Filter.prototype._encodeSub = function (info) {
	...
	for (var x = 0; x < info.scanLineLength; x++) {
		info.outputData[info.outputOffset + x + 1] = Math.abs(this._getPixel(info, x) - this._getLeftPixel(info, x));
	}
};
...
Filter.prototype._encodeUp = function (info) {
	...
	for (var x = 0; x < info.scanLineLength; x++) {
		info.outputData[info.outputOffset + x + 1] = Math.abs(this._getPixel(info, x) - this._getTopPixel(info, x));
	}
};
...
Filter.prototype._encodeAverage = function (info) {
	...
	for (var x = 0; x < info.scanLineLength; x++) {
		info.outputData[info.outputOffset + x + 1] = Math.abs(this._getPixel(info, x) - Math.floor((this._getLeftPixel(info, x) + this._getTopPixel(info, x)) / 2));
	}
};
...
Filter.prototype._encodePaeth = function (info) {
	...
	for (var x = 0; x < info.scanLineLength; x++) {
		info.outputData[info.outputOffset + x + 1] = Math.abs(
