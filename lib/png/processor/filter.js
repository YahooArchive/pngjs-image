...
Filter.prototype.reverse = function () {
	...
	input = {
		...
		scanLineLength: Math.ceil(headerChunk.getScanLineLength())
	};
	output = {
		previousLineOffset: null,
	...
	for (var y = 0; y < height; y++) {
		...
		filterType = input.data[input.offset]; input.offset++;
...
Filter.prototype._reverseSub = function (input, output) {
	for (var x = 0; x < input.scanLineLength; x++) {
		output.data[output.offset + x] = (this._getPixel(input, output, x) + this._getLeftPixel(input, output, x)) & 0xff;
	}
};
...
Filter.prototype._reverseUp = function (input, output) {
	for (var x = 0; x < input.scanLineLength; x++) {
		output.data[output.offset + x] = (this._getPixel(input, output, x) + this._getTopPixel(input, output, x)) & 0xff;
	}
};
...
Filter.prototype._reverseAverage = function (input, output) {
	for (var x = 0; x < input.scanLineLength; x++) {
		output.data[output.offset + x] = (this._getPixel(input, output, x) + Math.floor((this._getLeftPixel(input, output, x) + this._getTopPixel(input, output, x)) / 2)) & 0xff;
	}
};
...
Filter.prototype._getPixel = function (input, output, x) {
	return input.data[input.offset + x];
};
...
Filter.prototype._getLeftPixel = function (input, output, x) {
	return (x < input.bytesPerPixel) ? 0 : output.data[output.offset + x - input.bytesPerPixel];
};
...
Filter.prototype._getTopPixel = function (input, output, x) {
	return (output.previousLineOffset === null) ? 0: output.data[output.previousLineOffset + x];
};
...
Filter.prototype._getTopLeftPixel = function (input, output, x) {
	return ((output.previousLineOffset === null) || (x < input.bytesPerPixel)) ? 0 : output.data[output.previousLineOffset + x - input.bytesPerPixel];
};
