 ...
 */
Interlace.prototype.interlace = function (data, offset) {
 ...
 */
Interlace.prototype.reverse = function (data, offset) {

	var headerChunk = this.getHeaderChunk(),
		output;

	if (headerChunk.getInterlaceMethod() === interlace.ADAM7) {
		output = this._adam7(data, offset, headerChunk.getWidth(), headerChunk.getHeight(), true);
	} else {
		output = new Buffer(data.length - offset);
		data.copy(output, 0, offset, offset + output.length);
	}
