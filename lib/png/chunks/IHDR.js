...
module.exports = {
	...
	decode: function (data, offset, length, strict) {
		...
		this._bitDepth = data.readUInt8(position); position++; // 8
		this._colorType = data.readUInt8(position); position++; // 6
		this._compressionMethod = data.readUInt8(position); position++; // 0
		this._filterMethod = data.readUInt8(position); position++; // 0
		this._interlaceMethod = data.readUInt8(position); // 0
	},
