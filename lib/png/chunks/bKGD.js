...
var Scale = require('../processor/scale');
...
module.exports = {
	...
	parse: function (stream, length, strict) {
		...
		if (!headerChunk.isColor()) {
			...
			color = scale.scaleInValue(color);
			this.setColor({
				red: color,
				green: color,
				blue: color
			...
			this.setColor({
				red: scale.scaleInValue(stream.readUInt16BE()),
				green: scale.scaleInValue(stream.readUInt16BE()),
				blue: scale.scaleInValue(stream.readUInt16BE())
			});
		}
	},
	...
	compose: function (stream) {
		...
		if (!headerChunk.isColor()) {
			...
			color = scale.scaleOutValue(color);
			...
			color = this.getColor();
			stream.writeUInt16BE(scale.scaleOutValue(color.red));
			stream.writeUInt16BE(scale.scaleOutValue(color.green));
			stream.writeUInt16BE(scale.scaleOutValue(color.blue));
		}
	}
};
