 ...
 */
Chunk.prototype.encode = function (stream) {
 ...
 */
Chunk.prototype.decode = function (stream, length, strict) {
 ...
 *
 * Note:
 * Use this to modify image data
 *
 * @param {Buffer} image
 * @return {Buffer}
 */
Chunk.prototype.postDecode = function (image) {
...
Chunk.decodeTypeData = function (type, data, chunks, strict) {
	...
	if (methods && methods.decodeData) {
		methods._staticChunks = chunks;
		return methods.decodeData(data, strict);
	} else {
		throw new Error('Unknown chunk-type ' + type);
	}
};
...
Chunk.addChunkType = function (type, module) {
	...
	// Add utils
	utils.loadModule(path.join(__dirname, 'chunkUtils.js'), module);
