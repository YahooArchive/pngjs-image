...
module.exports = {
	...
	preDecode: function (data, offset, length, strict) {
		...
		if ((this.getLength() % 3) !== 0) {
			throw new Error('Palette length should be a multiple of 3. Length: ' + this.getLength());
		}
		...
		if (Math.pow(2, headerChunk.getBitDepth()) > (this.getLength() / 3)) {
	...
	decode: function (data, offset, length, strict) {
		this._palette = new Buffer(this.getLength());
		this._data.copy(this._palette, 0, this.getOffset(), this.getLength());
	},
	...
	getColor: function (index) {
		...
		return {
			r: data[internalIndex],
			g: data[internalIndex + 1],
			b: data[internalIndex + 2]
		}
	}
};
